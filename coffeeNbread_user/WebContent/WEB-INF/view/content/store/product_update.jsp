	...
	<sec:csrfInput/><%-- csrf 토큰 --%>
		<table>
			...
			<tr>
				<th>판매 여부</th>
				<td>
				<input type="checkbox" name="sellingOption" value="Y" <c:if test="${requestScope.product.sellingOption eq 'Y'}">checked="checked"</c:if>>판매함
			...
			<tr>
				<th>옵션</th>
				<td>
				<select name="optionCategoryString" id="optionCategoryString">
					<option>${requestScope.product.productCategory }</option>
					<c:forEach items="${requestScope.optionCategoryList }" var="optionCategory">
						<c:if test="${requestScope.product.productCategory ne optionCategory.optionCategory}">
			...
			<tr>
				<th>제품 사진</th>
				<td>
					<img src="/coffeeNbread_user/up_image/${requestScope.product.productPicture}" width="300px">
					<input type="hidden" name="OriginalProductPicture" value="${requestScope.product.productPicture}"/>
				</td>
