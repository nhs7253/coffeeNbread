...
public class PaymentDetailsServiceImpl implements PaymentDetailsService {
	 ...
	 * 장바구니에 있는거 결제내역에 등록
	 * 
	 * @Override public void addPaymentDetails(List<ShoppingBasketProduct>
	 * shoppingBasketProduct) throws NullShoppingBasketProductException {
	 * 
	 * System.out.println(shoppingBasketProduct);
	 * 
	 * if(shoppingBasketProduct==null){ throw new
	 * NullShoppingBasketProductException("장바구니가 비어있어 결제할 품목이 없습니다."); } else {
	 * for (int i = 0; i < shoppingBasketProduct.size(); i++) {
	 * 
	 * paymentDetailsDao.insertPaymentDetails(new PaymentDetails(0, "c", new
	 * Date(), "r", shoppingBasketProduct.get(i).getProductCount(),
	 * shoppingBasketProduct.get(i).getUserId(),
	 * shoppingBasketProduct.get(i).getProductId(),
	 * shoppingBasketProduct.get(i).getStoreId(), 0));
	 * 
	 * }
	 * 
	 * }
	...
	/*	*//**
			 ...
			 *//*
			 * 
			 * public void addUserHopedGetProductDate(List<PaymentDetails>
			 * paymentDetailsList, String userId, String storeId, Date
			 * tradeHopeDate) throws NotInsertTradeDateException {
			 * 
			 * Store store = sDao.selectStoreById(storeId); paymentDetailsList =
			 * pdDao.selectPaymentDetailsListByUserIdAndStoreId(userId,
			 * storeId);
			 * 
			 * 원하는 제품수령시간이 매장의 닫는시간 이후거나 , 매장의 오픈시간 이전이면 예외 생성 if
			 * (tradeHopeDate.compareTo(store.getStoreClose()) > 0 ||
			 * tradeHopeDate.compareTo(store.getStoreOpen()) < 0) { throw new
			 * NotInsertTradeDateException("매장 운영시간에 맞지 않아 수령할수있는 시간이아닙니다."); }
			 * for (int i = 0; i < paymentDetailsList.size(); i++) { update로
			 * 수정해야 함. dao.insertReservationDetails(new ReservationDetails(0,
			 * paymentDetailsList.get(i).getTradeDate(),
			 * paymentDetailsList.get(i).getReservationOrderCount(), null,
			 * tradeHopeDate, paymentDetailsList.get(i).getUserId(),
			 * paymentDetailsList.get(i).getProductId(),
